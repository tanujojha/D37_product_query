...
// or
db.products.find({product_material: /^s.*t$/i}).pretty()
// Find products which contain product color indigo  and product price 492.00
db.products.find({$and: [{product_color: "indigo"}, {product_price: 492.00}]}).pretty()
// Delete the products which product price value are same
db.products.find({}, {product_price: 1}).sort({_id: 1}).forEach((item) => {
    db.products.remove({_id: {$gt: item._id}, product_price: item.product_price})
});     // this delets only the duplicate products with same price

// or
db.products.find({}, {product_price: 1}).sort({_id: 1}).forEach((item) => {
    db.products.remove({_id: {$gt: item._id}, product_price: item.product_price})
});     // this delets all the products with same price 